...
class Grabber(object):
    ...
    Attributes:
        save_dir: Directory into which Grabber should download new images.
        link_path: Path to symlink to current image file. By default,
            `os.path.join(save_dir, "current")`
        library_dir: Directory MPD is currently playing from.
    ...
    def __init__(self, save_dir, library_dir, last_fm_api_key, link_path=None):
        ...
        if link_path is None:
            self.link_path = os.path.join(save_dir, "current")
        else:
    ...
    def get_art(self, song):
        ...
        Args: 
            song: A dictionary with keys 'album' and 'artist' corresponds to
                string representations of the album and artist (resp.) of 
    ...
    def _get_save_name(self, song, img_url=None):
        ...
        Args:
            song: A dictionary with keys 'album' and 'artist' corresponds to
                ...
                interest. Such a dictionary returns from 
                `MPDClient.currentsong()`
            img_url: A string representation of an external url points to the
                album artwork image file
        Returns: 
            `str` of the form 'Artist_Name_Album_Name_h45h.png', or if `img_url`
            is `None`, simply 'Artist_Name_Album_Name'. In this example,
            'h45h.png' extracts from the original filename in `img_url`.
        """
